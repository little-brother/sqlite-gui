...
bool isRequireHighligth = false, isRequireParenthesisHighligth = false, isRequireOccurrenceHighlight = false, isOccuranceHighligthed = false;
int selStartPos = -1;
	...
	switch (msg) {
		...
		case WM_COMMAND: {
			...
			if (cmd == IDM_QUERY_DATA || cmd == IDM_EDIT_DATA || cmd == IDM_ERASE_DATA || cmd == IDM_ADD_COLUMN || cmd == IDM_ADD_INDEX) {
				...
				if (cmd == IDM_QUERY_DATA) {
					...
					_sntprintf(query, len, TEXT("select * from %ls%ls%ls;\n"), isQ ? TEXT("\"") : TEXT(""), name16, isQ ? TEXT("\"") : TEXT(""));
					len = _tcslen(query);
				...
				if (cmd == IDM_ERASE_DATA) {
					int len = 256 + _tcslen(name16);
					TCHAR query16[len + 1];
					_sntprintf(query16, len, TEXT("Are you sure you want to delete all data from \"%ls\"?"), name16);
					if (MessageBox(hMainWnd, query16, TEXT("Delete confirmation"), MB_OKCANCEL) == IDOK) {
						_sntprintf(query16, len, TEXT("delete from \"%ls\";"), name16);
			...
			if (cmd == IDM_ADD || cmd == IDM_DELETE || cmd == IDM_VIEW || cmd == IDM_EDIT ||
				...
				if (cmd == IDM_DELETE) {
					...
					TCHAR msg16[256];
					_sntprintf(msg16, 255, TEXT("Are you sure you want to delete the %ls \"%ls\"?"), TYPES16[type], name16);
					if (MessageBox(hMainWnd, msg16, TEXT("Delete confirmation"), MB_OKCANCEL) == IDOK) {
						if (type == COLUMN) {
							...
							TCHAR query16[1024];
							_sntprintf(query16, 1023, TEXT("alter table \"%ls\" drop column \"%ls\""), tblname16, name16);
							char* query8 = utils::utf16to8(query16);
				...
				if (cmd == IDM_EDIT) {
					...
					TCHAR buf16[len + 1];
					_sntprintf(buf16, len, TEXT(" %ls"), name16);
		...
		case WM_NOTIFY: {
			...
			if (pHdr->hwndFrom == hEditorWnd && pHdr->code == EN_SELCHANGE) {
				...
				isRequireOccurrenceHighlight = (isRequireOccurrenceHighlight && pSc->seltyp == SEL_EMPTY) || isOccuranceHighligthed;
				if (!isRequireOccurrenceHighlight && pSc->seltyp != SEL_EMPTY) {
					int len = pSc->chrg.cpMax - pSc->chrg.cpMin;
					TCHAR* text = new TCHAR[len + 1];
					if (SendMessage(hEditorWnd, EM_GETSELTEXT, len + 1, (LPARAM)text)) {
						isRequireOccurrenceHighlight = true;
						for (int i = 0; i < len && isRequireOccurrenceHighlight; i++)
							isRequireOccurrenceHighlight = _istalnum(text[i]);
					}
					delete [] text;
				}
			...
			if (pHdr->hwndFrom == hTreeWnd && pHdr->code == TVN_ENDLABELEDIT) {
				...
				if (type == COLUMN) {
					...
					_sntprintf(query16, MAX_TEXT_LENGTH, TEXT("alter table \"%ls\" rename column \"%ls\" to \"%ls\""), tblname16, treeEditName, pMi->item.pszText);
				...
				if (type == TABLE) {
					_sntprintf(query16, MAX_TEXT_LENGTH, TEXT("alter table \"%ls\" rename to \"%ls\""), treeEditName, pMi->item.pszText);
				} else if (type > 1) {
					...
					_sntprintf(query16, MAX_TEXT_LENGTH,TEXT("drop %ls \"%ls\"; %ls;"), TYPES16[type], treeEditName, newDdl);
					delete [] ddl;
					delete [] newDdl;
				}
...
unsigned int __stdcall processEditorQuery (void* data) {
	...
	if (_tab.isBatch) {
		...
		SetWindowLongPtr(_tab.hTabWnd, GWLP_USERDATA, (LONG_PTR)hResultWnd); // Also see ACTION_UPDATETAB
...
int executeEditorQuery(bool isPlan, bool isBatch, bool onlyCurrent, int vkKey) {
	...
	if (tab->isBatch) {
		...
		for (int resultNo = 0; resultNo < queryCount; resultNo++) {
			...
			SetWindowLongPtr(hRowsWnd, GWLP_USERDATA, resultNo);
...
LRESULT CALLBACK cbNewResultPreview (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	if (msg == WM_COMMAND && wParam == IDM_EXPORT_FILE) {
		...
		_sntprintf(path, MAX_PATH, TEXT("image.%ls"), ext);
...
bool toggleComment (HWND hEditorWnd) {
	...
	for (int lineNo = lineStartNo; lineNo <= lineEndNo; lineNo++) {
		...
		if (opComment && (commentPos == -1)) {
			...
			TCHAR cLine[len + 3 + 3 + 1]{0};
			_sntprintf(cLine, len + 6, TEXT("-- %ls"), line);
